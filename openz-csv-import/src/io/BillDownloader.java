...
public class BillDownloader extends TimerTask {
	...
	public File getBills() throws IOException {
		...
		// usually only FileUtils.copyURLtoFile, but due to certificate problems
		// the certificate used cannot be verified.
		// Lengthy method start here
		/*
		 * HttpsURLConnection httpsCon = (HttpsURLConnection)
		 * url.openConnection(); httpsCon.setHostnameVerifier(new
		 * HostnameVerifier() {
		 * 
		 * @Override public boolean verify(String hostname, SSLSession session)
		 * { return true; } }); httpsCon.connect(); InputStream is =
		 * httpsCon.getInputStream(); FileUtils.copyInputStreamToFile(is,
		 * filename);
		 */
	 ...
	 *            Folder, where the files shall be stored.
	 */
